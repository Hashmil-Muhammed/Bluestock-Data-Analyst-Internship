...
def test_roe__normal(ratio_engine):
    assert ratio_engine.calc_roe({'net_profit':150, 'equity_capital':100, 'reserves': 900}) == 15.0
    
def test_roe_negative_equity(ratio_engine):
    assert ratio_engine.calc_roe({'net_profit':100, 'equity_capital' : 100, 'reserves':-200}) is None

def test_roce_normal(ratio_engine):
    assert ratio_engine.calc_roce({'operating_profit':250,'equity_capital':100, 'reserves':900, 'borrowings':1000}) == 12.5
    
# --- 3. Leverage & Efficiency Tests (Day 09) ---
def test_debt_to_equity_normal(ratio_engine):
    assert ratio_engine.calc_debt_to_equity({'company_id':'TCS', 'borrowings':500, 'equity_capital': 100, 'reserves': 900}) == 0.50
    
def test_debt_to_equity_bank_carve_out(ratio_engine):
    assert ratio_engine.calc_debt_to_equity({'company_id': 'HDFCBANK', 'borrowings': 5000, 'equity_capital':100, 'reserves': 900}) is None
...
def test_asset_turnover(ratio_engine):
    assert ratio_engine.calc_asset_turnover({'sales':2000, 'equity_capital': 500, 'reserves':300, 'borrowings':200}) == 2.0
    
def test_asset_turnover_zero_assets(ratio_engine):
    assert ratio_engine.calc_asset_turnover({'sales':2000, 'equity_capital':0, 'reserves':0, 'borrowings':0}) is None
...
def test_allocation_mature_cash_cow(cf_engine):
    assert cf_engine.get_allocation_pattern(cfo=1000, cfi=-500, cff=-200).startswith("1.")
    
def test_allocation_high_burn_startup(cf_engine):
    assert cf_engine.get_allocation_pattern(cfo=-1000, cfi=-500, cff=1500).startswith("6.")

def test_allocation_liquidity_crisis(cf_engine):
    assert cf_engine.get_allocation_pattern(cfo=-500, cfi=-200, cff=-100).startswith("5.")
