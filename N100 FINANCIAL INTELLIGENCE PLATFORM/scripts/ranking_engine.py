...
def build_ranking_engine():
    ...
    df['composite_score'] = (df['profitability_score'] * 0.5) + \
                            (df['growth_score'] * 0.3) + \
                            (df['valuation_score'] * 20) # Scaling valuation

    # 3. Ranking
    df = df.sort_values(by='composite_score', ascending=False)
    
    # Export to Excel
    df[['company_id', 'composite_score', 'ROE', 'Revenue_CAGR_3Y', 'pe_ratio']].to_excel(OUTPUT_FILE, index=False)
