...
def generate_radar_charts(db_path='nifty100.db', output_dir='reports/radar_charts'):
    ...
    with sqlite3.connect(db_path) as conn: 
        df = pd.read_sql("""
                         ...
                         """, conn)
    metrics_to_plot = ['ROE_Rank', 'ROCE_Rank', 'Net_Profit_Margin_Rank', 'PAT_CAGR_5Y_Rank', 'Debt_to_Equity_Rank']
    labels = ['ROE', 'ROCE', 'Net Profit Margin', 'PAT CAGR (5Y)', 'Debt safety']
    
    #Dynamically find the actual peer group column name
    peer_group_col = [col for col in df.columns if col.lower() not in ['company_id', 'year'] and not col.endswith('_Rank')][0]
