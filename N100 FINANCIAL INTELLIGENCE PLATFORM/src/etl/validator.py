    ...
    def validate_all(self, data_dict: dict) -> list:
        ...
        if comp_df is not None and not comp_df.empty:
            ...
            for _, row in duplicates.iterrows():
                self.log_failure('DQ-01', row['id'], None, 'id', 'Duplicate company ticker found', 'CRITICAL')
        ...
        if pl_df is not None and not pl_df.empty:
            if 'company_id' in pl_df.columns and 'year' in pl_df.columns:
                ...
                for _, row in duplicates.iterrows():
                    self.log_failure('DQ-02', row['company_id'], row['year'], 'PK', 'Duplicate P&L record for same year', 'CRITICAL')
            ...
            for _, row in bs_df.iterrows():
                if pd.notna(row.get('total_assets')) and pd.notna(row.get('total_liabilities')):
                    ...
                    if assets != 0 and (diff / assets) > 0.01:
                        self.log_failure('DQ-04', row['company_id'], row['year'], 'total_assets', 'Assets != Liabilities (>1% diff)', 'WARNING')
        ...
        if pl_df is not None and not pl_df.empty:
            for _, row in pl_df.iterrows():
                if pd.notna(row.get('sales')):
                    ...
                    if sales <= 0:
                        self.log_failure('DQ-06', row['company_id'], row['year'], 'sales', f'Sales is zero or negative ({sales})', 'WARNING')
        ...
        if self.failures:
            ...
            logger.warning(f"Validation finished with {len(self.failures)} issues. Saved to {output_path}")
        else:
...
if __name__ == "__main__":
    print("Validator logic is ready! Testing will be done during the full pipeline run.")
