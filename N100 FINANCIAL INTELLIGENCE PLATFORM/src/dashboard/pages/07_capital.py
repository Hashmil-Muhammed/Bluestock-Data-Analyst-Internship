...
def render_capital_allocation():
    st.set_page_config(page_title="Capital Allocation Map", page_icon="🧩", layout="wide")
    st.title("🧩 Capital Allocation Map")
    st.markdown("Explore how 92 companies are grouped based on their 8 Capital Allocation Patterns. **Click on a pattern to drill down and see the companies inside.**")
    st.markdown("---")
    ...
    pattern_col = next((c for c in df.columns if 'capital_allocation' in c or 'pattern' in c), None)
    ...
    with st.expander("View Raw Data"):
        st.dataframe(df[['company_id', 'company_name', pattern_col]], use_container_width=True)
