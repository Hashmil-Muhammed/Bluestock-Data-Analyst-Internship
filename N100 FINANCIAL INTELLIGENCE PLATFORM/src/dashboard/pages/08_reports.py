...
def render_annual_reports():
    ...
    st.markdown("View and download the annual reports of NIFTY 100 companies. Missing reports are highlighted with badges.")
    st.markdown("---")
    ...
    try:
        ...
        st.error(f"Error fetching data from database. Make sure the 'documents' table exists. Error: {e}")
        return
    ...
    pdf_col = next((c for c in df.columns if 'annual_report' in c or 'pdf' in c or 'link' in c or 'url' in c or 'bse' in c), None)
    ...
    df[year_col] = df[year_col].fillna('N/A').astype(str).str.replace('.0', '', regex=False)

    # 5. Badge Assignment for Missing Reports
    def get_status_badge(link):
        if pd.isna(link) or str(link).strip() == '' or str(link).lower() in ['none', 'nan', 'n/a']:
    ...
    with col1:
        companies_list = ["All Companies"] + sorted(display_df['Company Name'].dropna().unique().tolist())
        selected_company = st.selectbox("🔍 Filter by Company Name:", companies_list)
    with col2:
        status_filter = st.radio("Filter by Report Status:", ["All", "🟢 Available", "🔴 Missing Report"], horizontal=True)
