...
def render_valuation_analysis():
    ...
    st.markdown("Analyze historical P/E, P/B, EV/EBITDA trends, and rank companies by FCF & Dividend Yields.")
    st.markdown("---")
    ...
    try:
        ...
        st.error(f"Error fetching data from database. Check if tables exist. Details: {e}")
        return
    ...
    # ==========================================
    col_mapping = {
        'pe': next((c for c in df.columns if 'pe_ratio' in c or 'p_e' in c or c == 'pe'), None),
        'pb': next((c for c in df.columns if 'pb_ratio' in c or 'p_b' in c or c == 'pb'), None),
        'ev_ebitda': next((c for c in df.columns if 'ev_ebitda' in c or 'evebitda' in c), None),
        'roe': next((c for c in df.columns if 'roe' in c), None),
        'mkt_cap': next((c for c in df.columns if 'market_cap' in c or 'marketcap' in c), None),
        'sector': next((c for c in df.columns if 'sector' in c and c != 'company_id'), None),
        'year': next((c for c in df.columns if 'year' in c), None),
        
        # TASK 2: New columns for FCF & Dividend
        'fcf': next((c for c in df.columns if 'fcf' in c or 'free_cash_flow' in c), None),
        'div_yield': next((c for c in df.columns if 'dividend_yield' in c or 'div_yield' in c), None)
    }
    ...
    if not sec_c or not pe_c:
        st.error("Missing critical columns (Sector or P/E Ratio) for Valuation Analysis.")
        return
    ...
    companies = ["All Companies"] + sorted(df_filtered['company_name'].dropna().unique().tolist())
    ...
    with col1:
        ...
        if year_c and not df_filtered[year_c].isin(['N/A']).all():
            trend_data = df_filtered.groupby([year_c, 'company_name'])[pe_c].mean().reset_index()
            ...
            if unique_years > 1:
                fig_pe = px.line(
                    trend_data, x=year_c, y=pe_c, color='company_name',
                    markers=True, title="P/E Ratio Trend",
                    labels={year_c: "Financial Year", pe_c: "P/E Ratio"}
                )
            else:
                single_year = trend_data[year_c].iloc[0]
                fig_pe = px.bar(
                    trend_data, x='company_name', y=pe_c, color='company_name',
            ...
            fig_pe.update_layout(showlegend=(selected_company == "All Companies" and selected_sector != "All Sectors"))
    ...
    with col2:
        ...
        if pb_c and roe_c and mkt_c:
            valid_scatter = df_filtered[(df_filtered[pb_c] != 0) & (df_filtered[roe_c] != 0)]
            
            if not valid_scatter.empty:
                if year_c:
                    valid_scatter = valid_scatter.sort_values(year_c).drop_duplicates(subset=['company_name'], keep='last')
                ...
                fig_scatter = px.scatter(
                    ...
                    title=f"P/B vs ROE (Size: Market Cap)",
                    labels={pb_c: "Price to Book (P/B)", roe_c: "Return on Equity (ROE)"}
                )
    ...
    if ev_c:
        ...
        table_data = table_data.groupby(['company_name', sec_c])[ev_c].mean().reset_index()
        ...
        table_data['Premium_Flag'] = table_data.apply(
            lambda x: "⚠️ High Premium" if x[ev_c] > (x['Sector_Median_EV_EBITDA'] * 1.2) else "✅ Normal", 
            axis=1
        )
    ...
    st.markdown("Filter companies by Minimum Dividend Yield and rank them based on Free Cash Flow (FCF) Yield to identify value signals.")

    min_yield = st.slider("Select Minimum Dividend Yield (%) Threshold:", min_value=0.0, max_value=15.0, value=0.0, step=0.5)

    rank_df = df[df[sec_c] == selected_sector] if selected_sector != "All Sectors" else df.copy()

    if year_c and not rank_df.empty:
        rank_df = rank_df.sort_values(year_c).drop_duplicates(subset=['company_name'], keep='last')
    ...
    if not rank_df.empty:
        ...
        rank_df = rank_df.sort_values(by=['FCF Yield (%)', 'Dividend Yield (%)'], ascending=[False, False])
        rank_df.insert(0, 'Rank', range(1, 1 + len(rank_df)))

        st.dataframe(rank_df.round(2), width="stretch", hide_index=True)
    else:
        st.info(f"No companies found with a Dividend Yield of {min_yield}% or higher in the selected sector.")
    ...
    st.markdown("Identifies if a stock is trading at a Caution (High P/E) or Discount (Low P/E) compared to its sector.")
    ...
    if pe_c and sec_c:
        ...
        if year_c and not flag_df.empty:
            flag_df = flag_df.sort_values(year_c).drop_duplicates(subset=['company_name'], keep='last')
        ...
        # Display table
        display_flag_cols = ['company_name', sec_c, pe_c, 'Sector_Median_PE', 'Valuation_Flag']
        display_flag_df = flag_df[display_flag_cols].copy()
        display_flag_df.rename(columns={
            'company_name': 'Company Name',
            sec_c: 'Sector',
        ...
        with col_d1:
            ...
            st.download_button("Download Valuation Summary (CSV)", data=csv_summary, file_name='valuation_summary.csv', mime='text/csv')

        with col_d2:
            export_flags = display_flag_df[display_flag_df['Valuation_Flag'].str.contains("Caution|Discount")]
            csv_flags = export_flags.to_csv(index=False).encode('utf-8')
            st.download_button("Download Valuation Flags (CSV)", data=csv_flags, file_name='valuation_flags.csv', mime='text/csv')
    else:
        st.warning("P/E Ratio or Sector column is missing. Cannot compute overvaluation flags.")
