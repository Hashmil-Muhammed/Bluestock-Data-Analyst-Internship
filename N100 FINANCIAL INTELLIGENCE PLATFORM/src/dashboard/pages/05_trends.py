...
def render_trends():
    ...
    st.markdown("Analyze historical trends and Year-over-Year (YoY) growth across P&L and Balance Sheet.")
    st.markdown("---")
    ...
    selected_name = st.sidebar.selectbox("Select Company:", sorted(df_comp['company_name'].unique()))
    ...
    available_metrics = ['Sales', 'Operating_Profit', 'Net_Profit', 'Borrowings', 'Total_Assets', 'Net_Worth']
    ...
    if missing_cols:
        st.error(f"Error: The following columns are missing from the database query: {missing_cols}")
        return
    ...
    fig.update_layout(xaxis_title="Financial Year", yaxis_title="Amount (in Cr)", hovermode="x unified")
