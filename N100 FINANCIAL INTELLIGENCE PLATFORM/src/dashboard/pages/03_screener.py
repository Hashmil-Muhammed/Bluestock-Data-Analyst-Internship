...
def render_screener():
    ...
    selected_roe = st.sidebar.slider("Minimum ROE (%)", min_r, max_r, min_r, key="roe_slider")
    
    min_roc, max_roc = get_slider_range(df['ROCE'])
    selected_roce = st.sidebar.slider("Minimum ROCE (%)", min_roc, max_roc, min_roc, key="roce_slider")
    
    min_n, max_n = get_slider_range(df['NPM'])
    selected_npm = st.sidebar.slider("Minimum NPM (%)", min_n, max_n, min_n, key="npm_slider")
    
    min_d, max_d = get_slider_range(df['D_E'])
    selected_de = st.sidebar.slider("Maximum Debt to Equity", min_d, max_d, max_d, key="de_slider")
    ...
    st.dataframe(filtered_df.style.format(precision=2), use_container_width=True, height=400)
