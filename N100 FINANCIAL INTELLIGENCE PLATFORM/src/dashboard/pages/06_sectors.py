...
def render_sector_analysis():
    ...
    def clean_id(df):
        ...
        if 'id' in df.columns and 'company_id' not in df.columns:
            ...
            df = df.drop(columns=['id']) # id-യും company_id-യും ഉണ്ടെങ്കിൽ id ഒഴിവാക്കുന്നു
    ...
    col_mapping = {
        ...
        'sales': next((c for c in df.columns if ('sales' in c or 'revenue' in c) and 'cagr' not in c), None),
    ...
    if not df_sector.empty:
        ...
        fig_scatter = px.scatter(
            df_sector, x=sales_c, y=roe_c, 
            size=mkt_c if mkt_c else None, 
            color='company_name',
            hover_name='company_name',
            title=f"Revenue vs ROE (Size: Market Cap)"
        )
        ...
        if kpi_cols:
            ...
            fig_bar = px.bar(
                median_data, x='KPI', y='Median Value', 
                text_auto=True, color='KPI', 
