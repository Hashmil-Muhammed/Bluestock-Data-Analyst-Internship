...
def render_profile():
    ...
    if selected_company_name:
        ...
        comp_id = str(companies_df[companies_df['company_name'] == selected_company_name]['id'].iloc[0]).strip()
        ...
        if not kpi_df.empty:
            ...
            asset_turnover = format_metric(kpi_df['Asset_Turnover'].iloc[0], is_percentage=False)
            ...
            st.warning(f"⚠️ No valid calculated KPI data found for {selected_company_name}.")
        ...
        with chart_col1:
            ...
            if not pl_df.empty:
                fig_pl = px.bar(pl_df, x='year', y=['sales', 'net_profit'], barmode='group', 
                                title="P&L: Sales vs Net Profit", color_discrete_sequence=['#1f77b4', '#2ca02c'])
        ...
        with chart_col2:
            ...
            if not bs_df.empty:
                fig_bs = px.line(bs_df, x='year', y=['total_assets', 'total_liabilities'], 
                                 title="Balance Sheet Trends", color_discrete_sequence=['#ff7f0e', '#d62728'])
        ...
        with chart_col3:
            ...
            if not cf_df.empty:
                fig_cf = px.bar(cf_df, x='year', y=['operating_activity', 'investing_activity', 'financing_activity'], 
                                title="Cash Flow Breakdown", color_discrete_sequence=['#2ca02c', '#d62728', '#1f77b4'])
