...
def render_home():
    ...
    st.info("📊 **Market Health:** Nifty 100 aggregated financial health based on the latest database analysis.")
    ...
    with col1:
        avg_roe = kpi_df['avg_roe'].iloc[0] if not kpi_df.empty and kpi_df['avg_roe'].iloc[0] is not None else 0
        st.metric(label="Average ROE", value=f"{avg_roe:.2f}%")
        
    with col2:
        avg_npm = kpi_df['avg_npm'].iloc[0] if not kpi_df.empty and kpi_df['avg_npm'].iloc[0] is not None else 0
    ...
    if not sector_df.empty:
        ...
        st.warning("Sector data not available. Please check the database configuration.")
