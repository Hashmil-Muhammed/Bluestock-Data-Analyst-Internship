...
def score_sentiment():
    ...
    df['sentiment_score'] = df['text'].apply(lambda text: sia.polarity_scores(str(text))['compound'])
    ...
    if not df.empty:
        ...
        sample_df = pd.concat([
            df[df['type'] == 'Pro'].head(3),
            df[df['type'] == 'Con'].head(3)
        ])
        print(sample_df[['company_id', 'type', 'sentiment_score', 'text']].to_string(index=False))
