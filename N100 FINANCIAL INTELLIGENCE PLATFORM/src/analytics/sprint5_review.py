...
def run_sprint5_qa_review():
    ...
    if os.path.exists(tearsheets_dir):
        ...
        if len(pdf_files) > 0:
            ...
            print(f"    Action Required: Please manually open '{pdf_files[0]}' to ensure no text overlaps (UI Overflow).")
        else:
    ...
    if os.path.exists(pros_cons_file):
        ...
        if coverage >= 90:
            print(f"   SUCCESS: NLP Coverage is {coverage:.1f}% (Passes >90% requirement).")
        else:
            print(f"    WARNING: NLP Coverage is {coverage:.1f}% (Below 90% threshold).")
    else:
    ...
    if os.path.exists(cf_alerts_file):
        ...
        distress_count = len(df_alerts[df_alerts['Alert Badge'].str.contains('Distress', na=False, case=False)])
        print(f"    SUCCESS: CF Alerts file found. Spotted {distress_count} companies with 'Distress' signals.")
    else:
    ...
    if os.path.exists(cf_intel_file):
        ...
        if 'Capital Allocation Label' in df_intel.columns:
            ...
            print("    ERROR: Capital Allocation Matrix is missing from CF Intelligence.")
