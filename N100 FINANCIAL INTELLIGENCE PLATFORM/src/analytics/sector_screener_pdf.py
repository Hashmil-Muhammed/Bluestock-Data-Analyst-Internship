...
def generate_sector_and_screener_reports():
    ...
    roe_col = next((c for c in df_ratios.columns if c == 'roe'), next((c for c in df_ratios.columns if 'roe' in c), None))
    de_col = next((c for c in df_ratios.columns if c in ['d_e', 'debt_equity']), next((c for c in df_ratios.columns if 'debt' in c), None))
    opm_col = next((c for c in df_ratios.columns if c == 'opm'), next((c for c in df_ratios.columns if 'opm' in c), None))

    # Convert to numeric immediately to filter out completely blank rows
    for c in [roe_col, de_col, opm_col]:
        if c: df_ratios[c] = pd.to_numeric(df_ratios[c], errors='coerce')
    ...
    if 'year' in df_ratios_clean.columns:
        df_ratios_clean['year'] = df_ratios_clean['year'].astype(str).replace('PARSE_ERROR', '0000')
        df_ratios_clean = df_ratios_clean.sort_values(by=['company_id', 'year'], ascending=[True, False])

    latest_ratios = df_ratios_clean.drop_duplicates(subset=['company_id'], keep='first').copy()

    if latest_ratios.empty:
        print(" Error: No valid data found in financial_ratios after removing empty rows.")
        return

    sector_col = next((c for c in df_sectors.columns if 'sector' in c or 'industry' in c), None) if not df_sectors.empty else None
    
    if sector_col and not df_sectors.empty:
        df_merged = pd.merge(latest_ratios, df_sectors[['company_id', sector_col]], on='company_id', how='left')
    else:
    ...
    for sector in sectors:
        sector_safe_name = str(sector).replace('/', '_').replace('&', 'and').replace(' ', '_')
        ...
        if roe_col and de_col and not sector_data.empty:
            ...
            best_co = valid_roe_data.loc[valid_roe_data[roe_col].idxmax()] if not valid_roe_data.empty else None
            worst_co = valid_roe_data.loc[valid_roe_data[roe_col].idxmin()] if not valid_roe_data.empty else None
            ...
            if best_co is not None and worst_co is not None:
                elements.append(Paragraph("Performance Highlights (Based on ROE)", styles['Heading2']))
                highlight_table = [
                    ['Category', 'Company', 'ROE'],
                    [' Top Performer', str(best_co['company_id']), safe_fmt(best_co[roe_col], True)],
                    [' Lowest Performer', str(worst_co['company_id']), safe_fmt(worst_co[roe_col], True)]
                ]
    ...
    screener_files = [os.path.join(root_path, "output", "screener_output.xlsx"),
    ...
    if screener_file:
        ...
        if 'year' in temp_screen.columns and temp_screen['year'].astype(str).str.contains('PARSE', case=False, na=False).any():
            print(" Existing Screener file has PARSE_ERRORs. Regenerating clean data...")
        else:
    ...
    if df_screen.empty:
        ...
        comp_col = next((c for c in df_screen.columns if 'composite' in c or 'score' in c), None)
    ...
    if comp_col and not df_screen.empty:
        ...
        scr_elements.append(Paragraph("Screener Output Report - Top 10 Ranked Companies", title_style))
        ...
        for i, row in enumerate(df_screen.iterrows(), 1):
            _, r = row
            table_data.append([str(i), str(r['company_id']), safe_fmt(r[comp_col], False)])
        ...
        print(" Error: Could not generate Screener PDF (Missing Composite Score or empty data).")
