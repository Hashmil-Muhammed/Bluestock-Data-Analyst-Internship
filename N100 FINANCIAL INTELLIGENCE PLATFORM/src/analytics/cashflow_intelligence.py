...
def calculate_cfo_quality():
    ...
    if not os.path.exists(db_path):
        print(f" Error: Database nifty100.db not found.")
        return
    ...
    if 'year' not in df_pl.columns or 'year' not in df_cf.columns:
        print(" Error: 'year' column is missing from the tables. Cannot calculate 5-year average.")
        return
    ...
    pat_col = next((c for c in df_pl.columns if 'net profit' in c.replace('_', ' ') or 'pat' in c or 'profit after' in c.replace('_', ' ') or 'net_profit' in c), None)
    
    # Find Operating Cash Flow (CFO) column in Cashflow
    cfo_col = next((c for c in df_cf.columns if 'operating' in c.replace('_', ' ') or 'cfo' in c or 'cash from op' in c.replace('_', ' ') or 'cash_from' in c), None)

    if not pat_col:
        print(f" Error: Could not find Net Profit/PAT column in P&L. Available: {list(df_pl.columns)}")
        return
    if not cfo_col:
        print(f" Error: Could not find Operating Cash Flow column in Cashflow. Available: {list(df_cf.columns)}")
        return

    print(f" Using '{pat_col}' for PAT and '{cfo_col}' for CFO.")

    # Merge P&L and Cashflow on company_id and year
    df_merged = pd.merge(df_pl[['company_id', 'year', pat_col]], 
    ...
    df_merged = df_merged.sort_values(by=['company_id', 'year'], ascending=[True, False])
