...
def calculate_capex_and_fcf():
    ...
    try:
        ...
        df_existing['Company ID'] = df_existing['Company ID'].astype(str).str.strip().str.upper()
    ...
    ebitda_col = next((c for c in df_pl.columns if 'operating_profit' in c or 'ebitda' in c or 'op_profit' in c or ('operating' in c and 'profit' in c)), None)
    
    cfo_col = next((c for c in df_cf.columns if 'operating' in c or 'cfo' in c or 'cash_from' in c), None)
    ...
    if not all([sales_col, ebitda_col, cfo_col, capex_col]):
        ...
        print(f"Found -> Sales: {sales_col}, EBITDA: {ebitda_col}, CFO: {cfo_col}, CapEx: {capex_col}")
        return

    print(f" Using Columns -> Sales: '{sales_col}', EBITDA: '{ebitda_col}', CFO: '{cfo_col}', CapEx: '{capex_col}'")

    # Merge and Convert to Numeric
    df_merged = pd.merge(df_pl[['company_id', 'year', sales_col, ebitda_col]], 
    ...
    if not df_new.empty:
        ...
        cols_to_show = ['Company ID', 'Quality Badge', 'CapEx Label', 'FCF Conversion Label']
