...
class CashFlowEngine:
    ...
    def run(self):
        ...
        df['CapEx'] = df['cfi'].apply(lambda x: abs(x) if pd.notnull(x) and x < 0 else 0)
        ...
        df['CFO_Quality_Score'] = df.apply(
            lambda row: round(row['cfo'] / row['net_profit'], 2) if pd.notnull(row['net_profit']) and row['net_profit'] > 0 else None, 
            axis= 1
        )
        
        # 3. CapEx Intensity = CapEx / Sales
        df['CapEx_Intensity'] = df.apply(
            lambda row: round((row['CapEx'] / row['sales']) * 100, 2) if pd.notnull(row['sales']) and row['sales'] > 0 else None,
            axis=1 
        )
        
        # 4. FCF Conversion Ratio = FCF / Net Profit
        df['FCF_Conversion'] = df.apply(
            lambda row: round((row['FCF'] / row['net_profit']) * 100, 2) if pd.notnull(row['net_profit']) and row['net_profit'] > 0 else None,
            axis = 1
        )
...
if __name__ == "__main__":
    ...
    cols_to_show = ['company_id', 'year', 'FCF', 'CFO_Quality_Score', 'CapEx_Intensity', 'Capital_Allocation_Pattern']
