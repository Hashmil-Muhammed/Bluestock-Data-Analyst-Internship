...
def tag_business_description():
    ...
    def extract_clean_data(df, id_keywords, target_keywords):
        ...
        target_idx = next((i for i, c in enumerate(col_names) if any(k in c for k in target_keywords)), None)
    ...
    df_companies = extract_clean_data(df_companies_raw, id_kws, ['about', 'description', 'profile', 'business'])
    if 'target_text' not in df_companies.columns:
        print(" Error: Could not find 'about_company' column.")
        return
        
    df_sectors = extract_clean_data(df_sectors_raw, id_kws, ['sector', 'industry', 'macro'])
    ...
    # 4. NLP TAGGER LOGIC
    keyword_map = {
        'Financial Services': ['bank', 'finance', 'loan', 'credit', 'insurance', 'wealth', 'nbfc', 'capital', 'investment', 'mortgage', 'lending'],
        'IT': ['software', 'it services', 'technology', 'digital', 'consulting', 'cloud', 'cyber', 'tech', 'computing', 'data'],
        'FMCG': ['consumer', 'fmcg', 'food', 'personal care', 'beverages', 'retail', 'household', 'tobacco', 'cigarettes'],
        'Automobile': ['auto', 'vehicle', 'motor', 'car', 'two-wheeler', 'tractor', 'automotive', 'tyre'],
        'Healthcare': ['pharma', 'healthcare', 'medicine', 'drug', 'hospital', 'clinical', 'api', 'biotech', 'diagnostics'],
        'Energy': ['energy', 'oil', 'gas', 'power', 'petroleum', 'solar', 'coal', 'renewable', 'exploration', 'grid'],
        'Metals & Mining': ['steel', 'metal', 'mining', 'aluminium', 'iron', 'copper', 'zinc', 'cement'],
        'Construction': ['cement', 'construction', 'infrastructure', 'real estate', 'building', 'materials', 'realty'],
        'Telecom': ['telecom', 'network', 'broadband', 'wireless', 'communication', 'tower'],
        'Chemicals': ['chemical', 'agrochemical', 'polymers', 'specialty chemicals', 'fertilizer', 'paints']
    }
    ...
    for _, row in df_companies.iterrows():
        ...
        if predicted_sector != "Unknown" and actual_sector != "Not Found":
            if predicted_sector.lower() in actual_sector.lower() or actual_sector.lower() in predicted_sector.lower():
                is_match = True
            elif predicted_sector == 'IT' and 'information technology' in actual_sector.lower():
                is_match = True
            elif predicted_sector == 'Financial Services' and 'financial' in actual_sector.lower():
                is_match = True
            elif predicted_sector == 'FMCG' and 'consumer' in actual_sector.lower():
                is_match = True
                
        status = " MATCH" if is_match else (" MISMATCH" if predicted_sector != "Unknown" else " UNKNOWN")
        
        results.append({
            'Company ID': company,
            'Actual Sector': actual_sector,
            'Predicted Tag': predicted_sector,
            'Keywords Matched': ", ".join(found_keywords) if found_keywords else "None",
    ...
    if not df_results.empty:
        ...
        if mismatches > 0:
            ...
            print(df_results[df_results['Validation Status'] == ' MISMATCH'].head(5)[['Company ID', 'Actual Sector', 'Predicted Tag']].to_string(index=False))
