...
def calculate_fcf_cagr():
    ...
    cfo_col = next((c for c in df_cf.columns if 'operating' in c.replace('_', ' ') or 'cfo' in c or 'cash_from' in c), None)
    capex_col = None
    for c in df_cf.columns:
        if 'capex' in c or 'fixed_assets' in c or 'capital_expenditure' in c:
            ...
            break
    if not capex_col:
        capex_col = next((c for c in df_cf.columns if 'investing' in c.replace('_', ' ')), None)
    ...
    for company, group in df_cf.groupby('company_id'):
        ...
        cagr_5y = calc_cagr(fcf_5y_ago, latest_fcf, 5) if fcf_5y_ago is not None else "Insufficient Data"
        
        # 10-Year CAGR (Using data from 10 periods ago)
        fcf_10y_ago = group.iloc[10]['fcf'] if len(group) > 10 else None
        cagr_10y = calc_cagr(fcf_10y_ago, latest_fcf, 10) if fcf_10y_ago is not None else "Insufficient Data"
