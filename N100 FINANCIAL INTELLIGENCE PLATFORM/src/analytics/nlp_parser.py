...
def validate_cagr():
    ...
    for _, row in df_parsed.iterrows():
        ...
        if comp_data.empty:
            if company not in not_found_logs:
                print(f"DB Warning: Company '{company}' not found in financial_ratios table.")
        ...
        if "SALES" in metric or "REVENUE" in metric:
            expected_db_cols = [f"revenue_cagr_{period}y", f"sales_cagr_{period}y", f"revenue_cagr_{period}"]
        elif "PROFIT" in metric or "PAT" in metric:
            expected_db_cols = [f"pat_cagr_{period}y", f"profit_cagr_{period}y", f"pat_cagr_{period}"]
        else:
            continue 
        ...
        if actual_db_col:
            ...
            print(f"DB Warning: Could not find any column matching {expected_db_cols} for '{company}'.")
    ...
    if not df_results.empty:
        ...
        if not diverged.empty:
            ...
            print(diverged[['company_id', 'metric_type', 'period_years', 'absolute_diff']].to_string(index=False))
        else:
            print("Excellent! All cross-validated metrics match within the 5% threshold.")
    else:
