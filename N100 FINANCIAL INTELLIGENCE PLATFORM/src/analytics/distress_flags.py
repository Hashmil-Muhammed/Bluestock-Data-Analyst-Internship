...
def generate_distress_flags():
    ...
    cfo_col = next((c for c in df_cf.columns if 'operating' in c.replace('_', ' ') or 'cfo' in c), None)
    cff_col = next((c for c in df_cf.columns if 'financing' in c.replace('_', ' ') or 'cff' in c), None)
    ...
    print(f" Using Columns -> CFO: '{cfo_col}', CFF: '{cff_col}', Borrowings: '{borrowings_col}'")

    # 3. MERGE DATAFRAMES
    # Merge Cashflow and Balancesheet on company_id and year
    df_merged = pd.merge(df_cf[['company_id', 'year', cfo_col, cff_col]], 
    ...
    for company, group in df_merged.groupby('company_id'):
        ...
        prev_borrowings = group.iloc[1][borrowings_col] if len(group) > 1 else latest_borrowings
    ...
    if not df_results.empty:
        ...
        distress_count = len(df_results[df_results['Alert Badge'] == 'Distress Signal ⚠️'])
