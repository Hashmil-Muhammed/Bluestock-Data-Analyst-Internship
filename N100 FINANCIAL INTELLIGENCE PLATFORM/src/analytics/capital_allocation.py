...
def generate_capital_allocation():
    ...
    cfi_col = next((c for c in df_cf.columns if 'investing' in c.replace('_', ' ') or 'cfi' in c), None)
    ...
    for company, group in df_cf.groupby('company_id'):
        ...
        label = get_allocation_label(latest_record[cfo_col], latest_record[cfi_col], latest_record[cff_col])
        
        results.append({
            'Company ID': company,
            'Capital Allocation Label': label
        })
