...
def generate_portfolio_stats():
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='market_cap';")
    ...
    if not df_mc_latest.empty:
        df_merged = pd.merge(df_ratios_latest, df_mc_latest, on='company_id', how='left')
    else:
        df_merged = df_ratios_latest

    # 2. IDENTIFY TARGET COLUMNS DYNAMICALLY
    roe_col = next((c for c in df_merged.columns if 'roe' in c or 'return on equity' in c), None)
    de_col = next((c for c in df_merged.columns if 'd_e' in c or 'debt_equity' in c or 'debt to equity' in c), None)
    pe_col = next((c for c in df_merged.columns if 'p_e' in c or 'pe_ratio' in c or 'price to earnings' in c or 'pe' == c), None)
    ...
    for metric_name, col_name in target_metrics.items():
        if col_name and col_name in df_merged.columns:
            ...
            series = pd.to_numeric(df_merged[col_name], errors='coerce').replace([np.inf, -np.inf], np.nan).dropna()
