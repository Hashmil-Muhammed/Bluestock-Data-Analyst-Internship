...
def calculate_peer_percentiles(db_path= 'nifty100.db', peer_file='data/supporting/peer_groups.xlsx'):
    ...
    for metric, is_higher_better in metrics.items():
        if metric in df.columns:
            ...
            df[rank_col_name] = df.groupby(['year', peer_group_col])[metric].rank(pct=True, ascending=is_higher_better) * 100
