...
def run_kmeans_clustering():
    ...
    for col_name, var_name in [('ROE', roe_col), ('D/E', de_col), ('OPM', opm_col), 
                               ('Rev CAGR', rev_cagr_col), ('FCF CAGR', fcf_cagr_col)]:
        if not var_name:
            print(f" Warning: {col_name} not found. Injecting default 0.0 values to proceed.")
    ...
    def parse_mixed_numeric(val):
        ...
        if 'Turnaround' in val: return 15.0 # Proxy positive growth
        if 'Negative' in val: return -10.0 # Proxy negative growth
        if 'Insufficient' in val or 'N/A' in val: return np.nan
        try:
            return float(val)
        except ValueError:
            return np.nan
    ...
    distances = [np.linalg.norm(scaled_data[i] - centroids[clusters[i]]) for i in range(len(scaled_data))]
