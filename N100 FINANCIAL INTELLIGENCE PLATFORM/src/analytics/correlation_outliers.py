...
def generate_correlation_and_outliers():
    ...
    def clean_df(df):
        ...
        id_col = next((c for c in df.columns if c in ['id', 'company_id', 'cid', 'company', 'symbol']), None)
    ...
    sector_col = next((c for c in df_sectors.columns if 'sector' in c or 'industry' in c), None)
    if sector_col:
        df_merged = pd.merge(df_latest, df_sectors[['company_id', sector_col]], on='company_id', how='left')
    else:
    ...
    possible_kpis = ['roe', 'roce', 'd_e', 'npm', 'opm', 'icr', 'asset_turnover', 'revenue_cagr_3y', 'pat_cagr_3y', 'eps_cagr_3y']
    ...
    for metric in actual_kpis:
        ...
        sector_stats = df_merged.groupby(sector_col)[metric].agg(['mean', 'std']).reset_index()
        
        # Merge stats back to the main dataframe
        temp_df = pd.merge(df_merged[['company_id', sector_col, metric]], sector_stats, on=sector_col, how='left')
        
        # Calculate Z-score: (Value - Mean) / Std
        # Adding 1e-9 to prevent division by zero errors
        temp_df['z_score'] = (temp_df[metric] - temp_df['mean']) / (temp_df['std'] + 1e-9)
    ...
    if not df_outliers.empty:
        ...
        pd.DataFrame(columns=['company_id', 'sector', 'metric', 'value', 'sector_mean', 'sector_std', 'z_score', 'flag']).to_csv(outlier_file, index=False)
        print(f" No extreme outliers (|Z|>3) found. Clean report saved to {outlier_file}")
    ...
    try:
        ...
        print(f" Could not save as PNG (Kaleido issue). Saving as Interactive HTML instead...")
