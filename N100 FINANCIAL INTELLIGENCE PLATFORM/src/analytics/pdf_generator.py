...
def generate_pdf_reports():
    ...
    sales_col = next((c for c in df_pl.columns if c in ['sales', 'revenue']), next((c for c in df_pl.columns if 'sales' in c), None))
    pat_col = next((c for c in df_pl.columns if c in ['net_profit', 'pat']), next((c for c in df_pl.columns if 'profit' in c), None))
    
    roe_col = next((c for c in df_ratios.columns if c == 'roe'), next((c for c in df_ratios.columns if 'roe' in c), None))
    roce_col = next((c for c in df_ratios.columns if c == 'roce'), next((c for c in df_ratios.columns if 'roce' in c), None))
    ...
    for df, cols in [(df_pl, [sales_col, pat_col]), (df_ratios, [roe_col, roce_col, de_col, opm_col])]:
        for c in cols:
            if c and c in df.columns:
                df[c] = pd.to_numeric(df[c], errors='coerce')

    valid_companies = [c for c in df_pl['company_id'].unique() if not c.isnumeric() and c != 'NAN']
    companies = valid_companies[:3] if len(valid_companies) >= 3 else df_pl['company_id'].unique()[:3]
    ...
    for company in companies:
        ...
        company_ratios_clean = company_ratios.dropna(subset=[roe_col, roce_col, de_col], how='all').sort_values('year', ascending=False)
        ...
        if sales_col and pat_col and not company_pl.empty:
            ...
            x = range(len(years))
            plt.bar(x, sales, width=0.4, label='Revenue', color='#1f77b4', align='center')
            plt.bar([i + 0.4 for i in x], pat, width=0.4, label='Net Profit', color='#2ca02c', align='center')
        ...
        if not company_ratios_clean.empty:
            ...
            elements.append(Paragraph("Key Performance Indicators (Latest Valid Year)", styles['Heading2']))
    ...
    latest_ratios = df_ratios_clean.sort_values('year', ascending=False).drop_duplicates(subset=['company_id'])
