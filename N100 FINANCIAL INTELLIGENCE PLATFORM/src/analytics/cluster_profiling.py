...
def run_cluster_profiling():
    ...
    try:
        ...
        df_clusters['company_id'] = df_clusters['company_id'].astype(str).str.strip().str.upper()
        
        # Load Financial Ratios
        conn = sqlite3.connect(db_path if os.path.exists(db_path) else os.path.join(root_path, "data", "nifty100.db"))
    ...
    def clean_id(df):
        ...
        id_col = next((c for c in df.columns if c in ['id', 'company_id', 'cid', 'company', 'symbol']), df.columns[0])
    ...
    if 'year' in df_ratios.columns:
        df_ratios = df_ratios.sort_values(by=['company_id', 'year'], ascending=[True, False])
    ...
    de_col = next((c for c in df_merged.columns if 'd_e' in c or 'debt_equity' in c), None)
    opm_col = next((c for c in df_merged.columns if 'opm' in c), None)
    rev_cagr_col = next((c for c in df_merged.columns if 'revenue_cagr' in c or 'sales_growth' in c), None)
    fcf_cagr_col = next((c for c in df_merged.columns if 'fcf cagr' in c or 'fcf_cagr' in c), None)

    # Setup defaults for missing columns
    for col_name, var_name in [('ROE', roe_col), ('D/E', de_col), ('OPM', opm_col), ('Rev CAGR', rev_cagr_col), ('FCF CAGR', fcf_cagr_col)]:
        if not var_name:
            dummy_name = f"dummy_{col_name.lower()}"
            df_merged[dummy_name] = 0.0
            if col_name == 'ROE': roe_col = dummy_name
            elif col_name == 'D/E': de_col = dummy_name
            elif col_name == 'OPM': opm_col = dummy_name
            elif col_name == 'Rev CAGR': rev_cagr_col = dummy_name
            elif col_name == 'FCF CAGR': fcf_cagr_col = dummy_name

    def parse_mixed_numeric(val):
        if pd.isna(val): return 0.0
        val = str(val).strip()
        if '%' in val: return float(val.replace('%', ''))
        if 'Turnaround' in val: return 15.0 
        if 'Negative' in val: return -10.0 
        if 'Insufficient' in val or 'N/A' in val: return 0.0
    ...
    cluster_profiles = df_merged.groupby('cluster_id')[feature_cols].mean().reset_index()
    ...
    ranked_clusters = cluster_profiles.sort_values(by='health_score', ascending=False)['cluster_id'].tolist()
    ...
    for i, cid in enumerate(ranked_clusters):
        label_mapping[cid] = labels_ordered[i] if i < len(labels_ordered) else "Unknown Profile"
    ...
    display_profiles['Assigned Label'] = display_profiles['cluster_id'].map(label_mapping)
    display_cols = ['cluster_id', 'Assigned Label', roe_col, de_col, opm_col, rev_cagr_col, fcf_cagr_col]
